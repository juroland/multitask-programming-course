...
{
    (void)sig;
    for (int i = 0 ; i < MAX_BACKGROUND_GRP; ++i) {
        if (pid_grp[i] != 0) {
            kill(-pid_grp[i], SIGHUP);
        }
    }
    exit(0);
}
...
void sigchld_handler(int sig)
{
    ...
    for (int i = 0 ; i < MAX_BACKGROUND_GRP; ++i) {
        if (pid_grp[i] != 0) {
            if(waitpid(pid_grp[i], NULL, WNOHANG) > 0) {
                pid_grp[i] = 0;
            }
        }
    }
...
int pid_grp_insert(int pid)
{
    int i = 0;
    for ( ; i < MAX_BACKGROUND_GRP; ++i) {
        if (pid_grp[i] == 0) {
            pid_grp[i] = pid;
            break;
        }
    }
    if (i == MAX_BACKGROUND_GRP)
        return -1;
    else
        return i;
}
...
int main(void)
{
    ...
    while(getcmd(buf, sizeof(buf)) >= 0){
        if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
            ...
            char background = buf[strlen(buf)-2];
            if (background == '&') {
                buf[strlen(buf)-2] = 0;
                int pid = fork1();
                if(pid == 0) {
                ...
                if (pid_grp_insert(pid) == -1)
                    fprintf(stderr, "Unable to add this group in the background list..\n");
            } else {
                int pid = fork1();
                if(pid == 0) {
